...
단순히 주사위 굴림의 시각적 연출(Placebo)에 불과하다\"는 지적^1^을
극복하기 위해, 본 엔진은 모든 결과가 실제 물리 연산과 선수의 고유
능력치(Stats)에 의해 실시간으로 도출되는 **True 3D Simulation**을
지향합니다.
...
3.  **상태 기반 설계:** if-else 스파게티 코드를 지양하고, HFSM(계층적
    유한 상태 머신) 패턴을 사용하여 상태 관리를 명확히 한다.^2^
...
5.  **디버깅의 시각화:** 모든 AI의 판단(패스 경로 선택, 위치 선정
    이유)은 OnDrawGizmos를 통해 시각적으로 검증 가능해야 한다.^2^
...
좌표계 (-1.0, -1.0) \~ (1.0, 1.0)를 도입합니다.
  ...
  --------------------------------------------------------------------------
  **좌표 개념**     **정규화 좌표     **월드 좌표 변환     **비고**
                    (Normalized)**    공식                 
                                      (Implementation)**   
  ----------------- ----------------- -------------------- -----------------
  **Center Spot**   (0, 0)            FieldCenter          경기장 중앙

  **Top-Right**     (1.0, 1.0)        FieldCenter +        우측 상단 코너
                                      (FieldSize \* 0.5)   플래그

  **Penalty Spot**  (0, 0.8) (상대    FieldCenter +        페널티 마크 위치
                    진영)             (ForwardDir \*       
                                      FieldLength \* 0.4)  
...
통해 실제 좌표를 반환받도록 코드를 작성해야 합니다.^2^ 이는 경기장
프리팹이 변경되더라도 로직 수정 없이 AI가 올바른 위치를 찾아가게
보장합니다.
...
1.  **그리드 분할:** 경기장을
    ![](./images/media/image28.png){width="0.6479494750656168in"
    height="0.25081911636045495in"} 그리드로 논리적 분할합니다 (예:
    11vs11의 경우
    ![](./images/media/image2.png){width="0.6499015748031496in"
    height="0.24109251968503936in"} 그리드).
...
3.  **비용 주입:** NavMesh의 이동 비용(Cost)을 해당 셀의 점유율에
    비례하여 실시간으로 증가시킵니다.\
    ![](./images/media/image5.png){width="5.909749562554681in"
    height="0.5836789151356081in"}
...
메모리에 배치하는 구조체(Struct) 설계를 우선해야 합니다.^5^
...
떨어지는 드롭 슛(Dip Shot)은 모두 이 물리 법칙의 산물입니다.^4^

**물리 구현 공식 (Physics Implementation):**

![](./images/media/image6.png){width="6.243082895888014in"
height="0.75042104111986in"}

여기서:

- ![](./images/media/image20.png){width="9.130905511811023e-2in"
  height="0.2231988188976378in"} (공기 밀도): 기본값
  ![](./images/media/image8.png){width="1.0330402449693787in"
  height="0.25043416447944006in"} (날씨/고도에 따라 변동 가능^8^).

- ![](./images/media/image4.png){width="0.21915682414698162in"
  height="0.22959317585301836in"} (양력 계수): 공의 회전 속도에 따른
  비선형 계수.

- ![](./images/media/image27.png){width="0.13264982502187225in"
  height="0.23468832020997377in"} (각속도): 공의 회전 벡터.

- ![](./images/media/image22.png){width="0.10498031496062993in"
  height="0.24145450568678914in"} (속도): 공의 진행 벡터.
...
(MatchEngineConfig.Instance.DragCoefficient \* velocity.sqrMagnitude));\
}

이 코드는 FixedUpdate 내에서 매 프레임 실행되어야 하며, Rigidbody의 기본
Drag는 0으로 설정하고 스크립트에서 직접 제어합니다.^7^
...
- **Dry Grass:** 마찰 계수 높음
  (![](./images/media/image12.png){width="0.7843427384076991in"
  height="0.24053149606299212in"}). 공이 금방 멈춤.

- **Wet/Slick Grass:** 마찰 계수 낮음
  (![](./images/media/image19.png){width="0.7843427384076991in"
  height="0.24053149606299212in"}). 공이 미끄러지듯 빠르게 이동 (스키드
  현상).

충돌 감지 시 PhysicMaterial을 교체하거나, AddTorque를 통해 구름
저항(Rolling Resistance)을 시뮬레이션해야 합니다. 이는 패스 세기 조절
AI가 반드시 고려해야 할 변수입니다.^9^

### 3.3. 충돌과 물리적 경합 (Physical Duels)

선수 간의 충돌은 단순한 애니메이션 블렌딩이 아닙니다. 두 Rigidbody 간의
운동량 보존 법칙에 기반하되, \*\*선수 능력치(Stat)\*\*가 물리 법칙을
왜곡(Bias)시키는 형태여야 합니다.^2^

**경합 승리 확률 공식
(**![](./images/media/image11.png){width="0.45182305336832895in"
height="0.2521806649168854in"}**):**

![](./images/media/image14.png){width="6.243082895888014in"
height="0.5836603237095364in"}

![](./images/media/image17.png){width="6.243082895888014in"
height="0.8129560367454068in"}

이 공식은 ^2^ 문서의 \'물리 경합 성공률\'을 기반으로 하며, 승리 시
상대방은 밀려나거나(AddForce), 균형을 잃는 애니메이션(Stumble)이
재생됩니다. \'유리몸(Glass Body)\' 특성을 가진 선수는 충돌 충격량이
임계치를 넘을 경우 부상 확률이 급격히 증가합니다.^2^

## 4. 통계적 결정론: RPG 요소의 엔진 통합

모든 물리적 액션은 선수의 능력치(Stat)에서 시작됩니다. \"빠른 선수가
무조건 이기는\" 단순한 메타^1^를 방지하기 위해, 복합적인 스탯 상호작용
시스템을 구축합니다.
...
#### 실시간 능력치 계산 (![](./images/media/image15.png){width="0.6703291776027996in" height="0.2513735783027122in"}):

![](./images/media/image16.png){width="6.243082895888014in"
height="0.5836603237095364in"}

- **StateIndex (상태 지수):**
  ![](./images/media/image13.png){width="2.1615419947506562in"
  height="0.2494083552055993in"}. 체력과 호흡이 바닥나면 능력치가 50%로
  급감합니다.

- **MoraleMod (사기 보정):** 득점/실점 직후
  ![](./images/media/image25.png){width="0.986165791776028in"
  height="0.2387554680664917in"} 사이의 모멘텀 부여.

### 4.2. 오차 원뿔 시스템 (Error Cone System)

슈팅이나 패스의 정확도는 \'성공/실패\'의 이진 결과가 아닌, \*\*오차
범위(Error Radius)\*\*로 구현됩니다.^2^

- **Target Vector:** 목표 지점(골대 구석, 동료 발 앞).

- **Error Angle:**
  ![](./images/media/image23.png){width="2.1826148293963255in"
  height="0.24944116360454943in"}도.
...
선수는 현재 경기 상황에 따라 큰 틀의 행동 양식(State)을 결정합니다.^2^

  -----------------------------------------------------------------------
  **상위 상태 (Root       **하위 상태             **설명 및 전이 조건**
  State)**                (Sub-State)**           
  ----------------------- ----------------------- -----------------------
  **In Possession         BuildUp                 후방에서 공을 돌리며
  (공격)**                                        기회 모색 (수비 라인
                                                  높을 때)

                          Progression             미드필드 지역에서 전진
                                                  패스/드리블 시도

                          Penetration             파이널 서드 진입,
                                                  결정적 찬스 생성
                                                  (슈팅/킬패스)

  **Out of Possession     Pressing                공 가진 상대를 압박
  (수비)**                                        (전술 지침에 따라 강도
                                                  조절)

                          Block Formation         지정된 수비
                                                  구역(Zone)으로 복귀하여
                                                  대형 유지

                          Tracking Back           역습 당하는 상황에서
                                                  전력 질주로 복귀

  **Transition (전환)**   A-to-D                  공을 뺏긴 직후 3초.
                                                  즉시
                                                  압박(Gegenpressing)
                                                  또는 후퇴 결정
...
![](./images/media/image18.png){width="6.243082895888014in"
height="0.6566185476815398in"}

#### 예시: 패스 대상 선택 로직 (![](./images/media/image1.png){width="0.8382163167104112in" height="0.25146544181977254in"})

1.  **전진성 (Progressiveness):** 패스 시 골대와 얼마나 가까워지는가?
    (Weight: 0.4)

2.  **안전도 (Safety):** 패스 경로(Raycast) 상에 적이 없는가? 패스 성공
    확률 모델^12^ 활용. (Weight: 0.3)
...
점수가 급격히 0에 수렴하게 만듭니다.^10^ 단, \'중거리 슛\' 특성이 있는
선수는 이 곡선의 임계값이 더 넓게 설정됩니다.

### 5.3. 의사결정 주기 (Thinking Interval)

모든 AI가 매 프레임 판단을 내리면 CPU 부하가 걸릴 뿐만 아니라,
비현실적인 반응 속도를 보입니다. 따라서 **\'인지 지연(Reaction
Delay)\'** 시스템을 도입합니다.^2^

- **Think Tick:** 0.1s \~ 0.5s 가변 주기.

- **Formula:**
  ![](./images/media/image7.png){width="4.230635389326334in"
  height="0.25008639545056865in"}.
...
William Spearman의 모델^13^을 단순화하여 적용합니다. 경기장의 각 지점
![](./images/media/image24.png){width="0.11539698162729659in"
height="0.2517749343832021in"}를 누가 통제하고 있는지 확률적으로
계산합니다.

![](./images/media/image10.png){width="6.243082895888014in"
height="0.7608431758530184in"}

- ![](./images/media/image26.png){width="0.14583333333333334in"
  height="0.25in"}: 해당 지점까지 도달하는 데 걸리는 시간 (거리 / 속도 +
  반응시간).

- ![](./images/media/image21.png){width="0.11759514435695538in"
  height="0.25657042869641294in"}: 불확실성 계수.

이 히트맵(Heatmap)은 0.5초마다 갱신되며, 공격수 AI는
![](./images/media/image3.png){width="1.0870002187226597in"
height="0.2508464566929134in"}인 지점(아군이 확보한 공간)으로 침투하고,
수비수 AI는 \$PC(x)\$가 높은 적의 공간을 지우러 이동합니다.
...
- **라인 높이(Line Height):** 전술 설정^15^에 따라 수비 라인의 Z축
  Anchor가 조정됩니다.
...
골키퍼는 일반 필드 플레이어와 완전히 다른 로직을 가집니다.^2^
...
- **세이브 판정:** 시각적 충돌 이전에 수학적 판정 선행.\
  ![](./images/media/image9.png){width="5.920166229221348in"
  height="0.5836789151356081in"}\
...
# 부록: 핵심 물리 및 AI 상수 테이블 (Configuration Table)
  ...
  **파라미터 (Parameter)**                                            **권장값 (Default)**    **설명 및 출처**
  ------------------------------------------------------------------- ----------------------- -------------------------
  **Air Density                                                       1.225                   표준 대기압.
  (**![](./images/media/image20.png){width="9.130905511811023e-2in"                           날씨(비/눈)에 따라 변동
  height="0.2231988188976378in"}**)**                                                         가능 ^8^

  **Ball Mass**                                                       0.43 kg                 FIFA 공인구 표준 규격

  **Magnus Coeff                                                      0.0004 (approx)         회전수에 따른 양력 계수,
  (**![](./images/media/image4.png){width="0.21915682414698162in"                             BallAerodynamics.cs에서
  height="0.22959317585301836in"}**)**                                                        커브 조정 ^4^

  **Drag Coeff (Air)**                                                0.2 (Sphere)            공기 저항 계수.
                                                                                              스크립트에서 속도에 따라
                                                                                              가변 적용 ^19^

  **Ground Friction (Dry)**                                           0.8                     마른 잔디 마찰력. 공이 덜
                                                                                              구름.

  **Ground Friction (Wet)**                                           0.4                     젖은 잔디. 스키드 현상
                                                                                              발생.

  **Base Decision Tick**                                              0.2s                    AI 판단 주기 기본값.
                                                                                              Mental 스탯에 따라 가감
                                                                                              ^2^

  **Reaction Time (Human)**                                           0.7s                    피치 컨트롤 모델에서
                                                                                              사람의 인지 반응 속도
                                                                                              상수 ^13^

  **Max Sprint Speed**                                                9.8 m/s                 Speed 스탯 99일 때의 최대
                                                                                              속도 (약 35km/h)

  **Shot Error Noise**                                                ±15 deg                 Finishing 스탯 최하일 때
                                                                                              슈팅 각도 오차 범위 ^2^
  ---------------------------------------------------------------------------------------------------------------------
...
1.  The match engine is still placebo : r/footballmanagergames - Reddit,
    2월 19, 2026에 액세스,
    [[https://www.reddit.com/r/footballmanagergames/comments/1peylab/the_match_engine_is_still_placebo/]{.underline}](https://www.reddit.com/r/footballmanagergames/comments/1peylab/the_match_engine_is_still_placebo/)
...
4.  kiara-jacob/MagnusEffect_VPython: Magnus Effect - The \... - GitHub,
    2월 19, 2026에 액세스,
    [[https://github.com/kiara-jacob/MagnusEffect_VPython]{.underline}](https://github.com/kiara-jacob/MagnusEffect_VPython)
...
5.  An Evaluation of the Unity Machine Learning Agents Toolkit in Dense
    ...
    액세스,
    [[https://www.diva-portal.org/smash/get/diva2:1563588/FULLTEXT01.pdf]{.underline}](https://www.diva-portal.org/smash/get/diva2:1563588/FULLTEXT01.pdf)

6.  Unity\'s Data-Oriented Technology Stack (DOTS), 2월 19, 2026에
    액세스,
    [[https://unity.com/dots]{.underline}](https://unity.com/dots)

7.  Soccer Physics: CFD Analysis of the Magnus Effect - SimScale, 2월
    19, 2026에 액세스,
    [[https://www.simscale.com/blog/simulation-magnus-effect/]{.underline}](https://www.simscale.com/blog/simulation-magnus-effect/)

8.  2 The aerodynamics of the beautiful game 1 Introduction - MIT, 2월
    19, 2026에 액세스,
    [[https://thales.mit.edu/bush/wp-content/uploads/2013/11/Beautiful-Game-2013.pdf]{.underline}](https://thales.mit.edu/bush/wp-content/uploads/2013/11/Beautiful-Game-2013.pdf)
...
9.  Applied Physics and Technologies in Football, The Earth´s
    ...
    the, 2월 19, 2026에 액세스,
    [[https://mjbas.com/data/uploads/83209.pdf]{.underline}](https://mjbas.com/data/uploads/83209.pdf)
...
12. (PDF) Physics-Based Modeling of Pass Probabilities in Soccer -
    ...
    [[https://www.researchgate.net/publication/315166647_Physics-Based_Modeling_of_Pass_Probabilities_in_Soccer]{.underline}](https://www.researchgate.net/publication/315166647_Physics-Based_Modeling_of_Pass_Probabilities_in_Soccer)
...
14. Pitch Control --- Finding The Right Pass \| by Ishdeep Chadha -
    ...
    [[https://medium.com/@ishdeepsinghchadha/pitch-control-finding-the-right-pass-40e42323ceab]{.underline}](https://medium.com/@ishdeepsinghchadha/pitch-control-finding-the-right-pass-40e42323ceab)

15. Defensive Line Height: The Metric Defining Modern Defending - The
    \..., 2월 19, 2026에 액세스,
    [[https://the-footballanalyst.com/defensive-line-height-the-metric-defining-modern-defending/]{.underline}](https://the-footballanalyst.com/defensive-line-height-the-metric-defining-modern-defending/)

16. Zonal marking: football tactics explained - Coaches\' Voice, 2월 19,
    2026에 액세스,
    [[https://learning.coachesvoice.com/cv/zonal-marking-football-tactics-explained-mourinho-conte-simeone/]{.underline}](https://learning.coachesvoice.com/cv/zonal-marking-football-tactics-explained-mourinho-conte-simeone/)

17. Improving Dribbling, Passing, and Marking Actions in Soccer \...,
    2월 19, 2026에 액세스,
    [[https://arxiv.org/abs/2401.03406]{.underline}](https://arxiv.org/abs/2401.03406)

18. Scoring smarter goals with the help of AI in soccer - Ultralytics,
    2월 19, 2026에 액세스,
    [[https://www.ultralytics.com/blog/scoring-smarter-goals-with-the-help-of-ai-in-soccer]{.underline}](https://www.ultralytics.com/blog/scoring-smarter-goals-with-the-help-of-ai-in-soccer)

19. The drag force on an American football, 2월 19, 2026에 액세스,
    [[http://users.df.uba.ar/sgil/physics_paper_doc/papers_phys/fluids/drag_football.pdf]{.underline}](http://users.df.uba.ar/sgil/physics_paper_doc/papers_phys/fluids/drag_football.pdf)
